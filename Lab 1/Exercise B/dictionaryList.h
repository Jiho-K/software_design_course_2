...
public:
  ...
  // PROMISES: size() == 0.
	...
	Datum& operator [] (const int index);
	// REQUIRES: index at which user would like to find the string at
  // PROMISES: retruns the Mystring which corresponds to the index
	
	friend ostream& operator << (ostream& os, DictionaryList& S);
	// REQUIRES: S refers to the DictionaryList object which user would like to output
	// os refers to the out stream
